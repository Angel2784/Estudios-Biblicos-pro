...
export default function AnnotationReader({ texto, cita }: Props) {
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    if (target) {
      ...
      setSelectedColor(target.dataset.color ?? '#fbbf24')
    ...
    if (!sel || sel.isCollapsed) {
      if (!toolbarRef.current?.contains(e.target as Node)) {
        setToolbar(null); setActiveId(null)
      }
      return
    }
  ...
  const applyHighlight = useCallback((): string | null => {
    const sel = window.getSelection()
  ...
  const handleNota = () => {
    let id = activeId
    if (!id) { id = applyHighlight(); if (!id) return }
    const an = anotaciones.find(a => a.id === id)
    setModal({ id, preview: (an?.fragmento ?? '').substring(0, 60), nota: an?.nota ?? '' })
    setToolbar(null)
  }
  ...
  const removeHighlight = () => {
    ...
    setActiveId(null); setToolbar(null)
  }
  ...
  const toolbarLeft = toolbar ? Math.min(toolbar.x + 8, (typeof window !== 'undefined' ? window.innerWidth : 800) - 360) : 0
