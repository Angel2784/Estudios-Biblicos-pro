...
export default function ApiKeySetup({ onSave }: Props) {
  ...
  const testKey = async () => {
    ...
    try {
      ...
      else { const d = await res.json(); setError(d?.error?.message || 'API Key inválida') }
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            ...
            <button onClick={testKey} disabled={!key.trim() || testing} className="btn-primary w-full justify-center">
              {testing ? <><span className="animate-spin">⟳</span> Verificando...</> : <><Zap size={16} /> Conectar y comenzar</>}
            </button>
          </div>
