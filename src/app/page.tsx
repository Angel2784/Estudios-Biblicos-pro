...
import { Settings, BookOpen, Copy, Bookmark, Download, ChevronDown, X, MessageSquare } from 'lucide-react'
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <header className="flex justify-between items-center bg-[#131b2e] border border-[#d4af37]/30 p-4 rounded-2xl shadow-xl">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button className="flex items-center space-x-1.5 bg-[#1e293b] hover:bg-[#273548] border border-[#d4af37]/30 px-4 py-1.5 rounded-xl text-sm font-medium transition text-gray-200">
              <BookOpen size={16} />
              <span>Biblioteca</span>
            </button>
            <button className="p-2 bg-[#1e293b] hover:bg-[#273548] border border-[#d4af37]/30 rounded-xl transition text-gray-200">
              <Settings size={18} />
            </button>

            {isLoaded && user ? (
              <div className="flex items-center space-x-2 bg-[#1e293b] border border-[#d4af37]/30 px-3 py-1.5 rounded-xl">
                <UserButton afterSignOutUrl="/" />
                <span className="text-xs text-gray-200 font-medium">{user.firstName || 'Usuario'}</span>
              </div>
            ) : (
              <div className="flex items-center space-x-2">
                <a href="/sign-in" className="text-xs bg-[#1e293b] hover:bg-[#273548] border border-[#d4af37]/30 px-3 py-2 rounded-xl font-medium text-[#f3e5ab]">
                  Iniciar Sesión
                </a>
                <a href="/sign-up" className="text-xs bg-gradient-to-r from-[#d4af37] to-[#aa7c11] text-black font-bold px-3 py-2 rounded-xl shadow">
                  Registrarse
                </a>
              </div>
            )}
          ...
          <div className="bg-[#131b2e] border border-[#d4af37]/40 p-6 rounded-2xl shadow-2xl space-y-4">
            <form onSubmit={handleEstudiar} className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="bg-gradient-to-r from-[#d4af37] to-[#aa7c11] text-black font-bold px-6 py-3 rounded-xl shadow-lg hover:opacity-90 transition disabled:opacity-50 cursor-pointer"
              >
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {['Juan 3:16', 'Salmos 23:1-6', 'Romanos 8:28', 'Filipenses 4:13'].map((item) => (
                <button
                  ...
                  className="text-xs bg-[#1e293b] hover:bg-[#273548] border border-[#d4af37]/30 text-[#f3e5ab] px-3.5 py-1.5 rounded-full transition cursor-pointer"
                >
                  {item}
                </button>
              ))}
            ...
            <div className="bg-[#2a1215] border border-red-500/40 text-red-200 text-xs p-3 rounded-xl flex items-center justify-between">
              ...
              <a href="/sign-up" className="underline font-bold hover:text-white">Mejorar plan</a>
            </div>
          </div>
        </section>
        ...
        {resultado && (
          <div className="bg-[#131b2e] border border-[#d4af37]/40 p-6 rounded-2xl shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-[#d4af37]/20 pb-3">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={() => navigator.clipboard.writeText(resultado)} className="flex items-center space-x-1 text-xs bg-[#1e293b] border border-[#d4af37]/30 px-3 py-1.5 rounded-lg hover:bg-[#273548] text-gray-200">
                  <Copy size={14} /> <span>Copiar todo</span>
                </button>
                <button className="flex items-center space-x-1 text-xs bg-[#1e293b] border border-[#d4af37]/30 px-3 py-1.5 rounded-lg hover:bg-[#273548] text-gray-200">
            ...
            <div className="flex items-center justify-between bg-[#1e293b] border border-[#d4af37]/20 px-4 py-2.5 rounded-xl text-sm cursor-pointer">
        ...
        <section className="space-y-4">
          ...
          <div className="bg-[#131b2e] border border-[#d4af37]/40 p-6 rounded-2xl shadow-2xl space-y-4">
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-[#d4af37] to-[#aa7c11] text-black font-bold py-3 rounded-xl shadow-lg hover:opacity-90 transition cursor-pointer"
            >
              Comparar
            </button>
