...
import { Library, Settings, Flame, Search, X } from 'lucide-react'
...
export default function HomePage() {
  ...
  const handleStudy = async () => {
    ...
    try {
      ...
      setEstudios(prev => [{ id: uid(), cita: citaInput.trim(), texto }, ...prev])
      setCitaInput('')
    } catch (e: unknown) {
      setErrorExeg(e instanceof Error ? e.message : 'Error desconocido')
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      setErrorComp(e instanceof Error ? e.message : 'Error desconocido')
    } finally { setComparando(false) }
  }

  const handleSelectEstudio = (estudio: EstudioGuardado) => {
    setEstudios(prev => [{ id: uid(), cita: estudio.cita, texto: estudio.texto }, ...prev])
  ...
  // ── Handlers drag tarjetas estudios ─────────────────────────────────────
  const onEstudioDragStart = (idx: number) => { dragEstudio.current = idx }
  const onEstudioDragOver  = (e: React.DragEvent) => e.preventDefault()
  const onEstudioDrop      = (idx: number) => {
    if (dragEstudio.current === null || dragEstudio.current === idx) return
    setEstudios(prev => reorder(prev, dragEstudio.current!, idx))
    dragEstudio.current = null
  }

  // ── Handlers drag tarjetas comparados ───────────────────────────────────
  const onComparadoDragStart = (idx: number) => { dragComparado.current = idx }
  const onComparadoDragOver  = (e: React.DragEvent) => e.preventDefault()
  const onComparadoDrop      = (idx: number) => {
    if (dragComparado.current === null || dragComparado.current === idx) return
    setComparados(prev => reorder(prev, dragComparado.current!, idx))
    dragComparado.current = null
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--navy)' }}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-8">
        ...
        {/* SECCIÓN 1: EXÉGESIS */}
        <section>
          <h2 className="font-bold text-lg mb-4" style={{ fontFamily: 'Crimson Pro, serif' }}>
            📖 Estudio Bíblico Pro
          </h2>
          <div className="card">
            <div className="flex gap-3 flex-col sm:flex-row">
              <input
                ...
                disabled={estudiando}
              />
              <button className="btn-primary whitespace-nowrap" onClick={handleStudy}
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {EJEMPLOS.map(ej => (
                <button key={ej} className="tab-btn" style={{ fontSize: 11 }} onClick={() => setCitaInput(ej)}>{ej}</button>
              ))}
            </div>
            {errorExeg && (
            ...
            {estudiando && (
              <div className="mt-4 space-y-3">
                ...
                <p className="text-xs" style={{ color: 'var(--text-dim)' }}>✨ Generando exégesis académica...</p>
              </div>
            )}
          </div>
          ...
          {estudios.map((e, i) => (
            <div
              key={e.id}
              draggable
              onDragStart={() => onEstudioDragStart(i)}
              onDragOver={onEstudioDragOver}
              onDrop={() => onEstudioDrop(i)}
              style={{ cursor: 'grab' }}
            >
              <StudySection
                cita={e.cita}
                texto={e.texto}
                onClear={() => setEstudios(prev => prev.filter(x => x.id !== e.id))}
              />
            </div>
          ))}
        </section>
        ...
        <section>
          ...
          <div className="card">
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {COMP_EJEMPLOS.map(([c1, c2]) => (
                <button key={`${c1}-${c2}`} className="tab-btn" style={{ fontSize: 11 }}
                  onClick={() => { setCita1(c1); setCita2(c2) }}>{c1} vs {c2}</button>
              ))}
            </div>
            {errorComp && (
              <div className="mt-3 p-3 rounded-lg text-xs" style={{ background: '#7f1d1d33', color: '#ef4444', border: '1px solid #7f1d1d' }}>
            ...
            {comparando && (
              <div className="mt-4 space-y-3">
                ...
                <p className="text-xs" style={{ color: 'var(--text-dim)' }}>🔍 Generando análisis comparativo...</p>
              </div>
            )}
          </div>
          ...
          {comparados.map((c, i) => (
            <div
              key={c.id}
              draggable
              onDragStart={() => onComparadoDragStart(i)}
              onDragOver={onComparadoDragOver}
              onDrop={() => onComparadoDrop(i)}
              style={{ cursor: 'grab' }}
            >
              <ComparativeSection
                cita1={c.cita1}
                cita2={c.cita2}
                texto={c.texto}
                onRemove={() => setComparados(prev => prev.filter(x => x.id !== c.id))}
              />
            </div>
          ))}
        </section>
      ...
      {showLibrary && (
        <>
          ...
          <LibrarySidebar
            onSelectEstudio={handleSelectEstudio}
            onSelectComparado={handleSelectComparado}
