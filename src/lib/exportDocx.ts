...
/** Descarga un Blob sin librerías externas */
...
export async function exportarComoWord(cita: string, texto: string): Promise<void> {
  ...
  descargarBlob(blob, `Exegesis_${cita.replace(/[:\s]/g, '_')}.docx`)
}
...
export async function exportarBibliotecaWord(estudios: EstudioGuardado[]): Promise<void> {
  ...
  estudios.forEach((e, idx) => {
    ...
    allParagraphs.push(...buildStudyParagraphs(e.cita, e.texto, fecha))
  ...
  descargarBlob(blob, `Biblioteca_Biblica_${fecha}.docx`)
}
