...
const FORMATO_SECCIONES = `REGLA DE FORMATO CRÍTICA: Cada sección DEBE comenzar exactamente con el número, un punto y un espacio (ejemplo: "1. Título"). NO uses almohadillas (#), NO uses asteriscos (**), NO uses ningún markdown en los títulos de sección. El contenido de cada sección sí puede usar markdown normal.`
...
Responde EXACTAMENTE con estas 13 secciones numeradas, usando este formato literal:
...
Responde EXACTAMENTE con estas 10 secciones numeradas, usando este formato literal:

1. Textos Bíblicos
Presenta ambos textos completos en español (RVR60 o NVI).

2. Similitudes Temáticas
Temas, imágenes y conceptos que comparten ambos pasajes.

3. Diferencias Significativas
Contrastes doctrinales, de énfasis o de contexto entre los pasajes.

4. Contexto Histórico Comparado
Época, autor, audiencia y situación de cada pasaje.

5. Análisis Lingüístico Comparado
Palabras clave en hebreo o griego de ambos pasajes con sus matices.

6. Progresión Teológica
¿Cómo avanza la revelación de uno al otro? ¿Hay cumplimiento o desarrollo?

7. Tensiones Aparentes
Posibles contradicciones y su resolución hermenéutica.

8. Síntesis Doctrinal
Doctrina unificada que emerge de leer ambos pasajes juntos.

9. Aplicación Integrada
Principios prácticos derivados de la comparación.

10. Conclusión Hermenéutica
Mensaje central y método de interpretación aplicado.`
  const { text } = await generateContent(apiKey, prompt)
  return text
}
...
Responde EXACTAMENTE con estas 8 secciones numeradas, usando este formato literal:

1. Introducción
Gancho inicial, contexto del pasaje y presentación del tema central.

2. Punto Central 1
Primer punto del sermón con desarrollo exegético y aplicación.

3. Punto Central 2
Segundo punto del sermón con desarrollo exegético y aplicación.

4. Punto Central 3
Tercer punto del sermón con desarrollo exegético y aplicación.

5. Ilustraciones y Analogías
Historias, ejemplos o metáforas que ilustran los puntos principales.

6. Aplicación Práctica
Pasos concretos y específicos que la audiencia puede tomar esta semana.

7. Llamado y Conclusión
Llamado a la acción, resumen del mensaje y cierre impactante.

8. Oración Sugerida
Oración de cierre o de respuesta al mensaje.`
  const { text } = await generateContent(apiKey, prompt)
  return text
}
