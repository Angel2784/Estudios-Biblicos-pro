...
/** Parte un string largo en trozos de máximo `maxLen` chars, cortando en espacios */
const wrapLine = (linea: string, maxLen = 100): string[] => {
  ...
  while (restante.length > maxLen) {
    ...
    if (corte <= 0) corte = maxLen // Si no hay espacio, cortar duro
 ...
 * Convierte el campo "texto" de string a array de líneas cortas (≤100 chars)
 * para que el JSON exportado sea legible verticalmente sin scroll horizontal.
 */
const replacerExport = (_key: string, value: unknown): unknown => {
  if (_key === 'texto' && typeof value === 'string') {
    // Dividir por saltos de línea y luego envolver líneas largas
    return value
      .split('\n')
      .flatMap((linea) => wrapLine(linea, 100))
  }
  return value
}
